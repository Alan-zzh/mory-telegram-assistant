...
def _generate_late_night_warning(ai, uname, is_group, uid):
    ...
    try:
        ...
        prompt = (
            f"你是 Mory 小助理，人设是温柔俏皮、不套路、说话像朋友一样自然。\n\n"
            f"现在是凌晨，用户{uname}还在群里闲聊没睡觉。你随口提一句去睡觉。\n\n"
            f"要求：\n"
            f"1. 20-30字，像熟人半夜随口劝一句，可以带点俏皮，不说教\n"
            f"2. 带一点温和的关心，不撒娇、不暧昧、不假设关系\n"
            f"3. 可以提到：熬夜伤身/明天没精神/早点休息/明天再聊\n"
            f"4. 结尾要有emoji（😴💤🌙✨选一个）\n"
            f"5. seed={seed}，每次必须不同\n\n"
            f"禁止：\n"
            f"- 长辈式说教语气（如'你应该'、'你必须'、'身体要紧'）\n"
