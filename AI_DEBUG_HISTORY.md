...
- 问题 | 广告号用“同城PC+担保交易”每隔数小时重复刷屏，入群显示名含“同程嫖娼”仍放行。根因 | 成人规则缺少 PC 组合语义，重复清理只覆盖一小时三次。解法 | 显示名与正文各加独立高置信组合；既有 kicked 状态不改写并补齐统一持久态。预防 | 固定截图正例、电脑/反诈反例和 kicked 补处置回归。
...
### 6.25 预设问题落入模型后连续乱答
- 问题 | 高频业务问法漏入模型或因同词串到错误答案，普通回复还可能混入销售事实。根因 | 预设只覆盖紧凑词形、部分规则用子串判断，风格样本又跨场景注入。解法 | 零Token问答族改为完整意图匹配、同题追问，并隔离含事实/CTA的全局样本。预防 | 每族固定自然问法、追问及跨话题反例矩阵。
...
### 66. health 绿灯被误当版本、调度与业务成功
- 问题|根因|解法|预防：health 200 时仍可能运行旧版本、保留历史调度失败或业务不可用；部署与巡检曾从 health 猜版本。版本改为直读 VPS `version.py`，调度分开当前执行/事务四态/历史 metrics，并强制业务探针；health 永远只判 liveness。

### 67. VPS timer 套用开发机 SSH/Git 前提，安装成功但实跑全失败
- 问题|根因|解法|预防：巡检 unit 已启用却返回 2/3/2；发布目录无 Git/内部文档/VPS SSH 凭据。生产目录自动改走本机只读执行，漂移核对发布子集和哈希，月审降级到 CHANGELOG；安装验收必须实跑全部 profile，不能只看 timer active。
