...
- 解法：VPS 逐源探测后删除三条稳定 403 直连，接入返回 200 的 NewsNow 头条/澎湃/早报；按来源权重、榜单位置、类目和单源上限选 10 条，科技≤1、财经≤2；AI/真实标题兜底/HTML/Rich 全链统一 10+1，AI 条数不合格直接回退；只接管 getUpdates 的 5xx/连接超时并做 1/2/4/8/15 秒退避和限频日志，sendMessage 等业务错误继续抛出。
