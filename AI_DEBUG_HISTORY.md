...
- 预防：追踪不等于定罪。任何历史删除都必须有逐条广告证据；保护内容导致无法复核正文时只能安全跳过，禁止用消息 ID 范围猜测删除。
### 44. O/o 混写日收益短句绕过数字规范化和赚钱承诺规则
- 问题：同日账号分别发送“一日 4oO+”“一日 9Oo+”，检测均为 0 分，未删除也未禁言。
- 根因：规范化只处理全角/数学数字，不处理数字串中的 O/o；赚钱规则覆盖日入、一天数字和 w/k/米组合，却没有“一日/每日+数字+加号”模板。
- 解法：仅把紧邻加号的混写数字串 O/o 转为 0，再用日周期+数字+加号高置信规则首条处置；同时收紧旧“一天+数字”规则的计量单位边界。
- 预防：收益黑话回归同时覆盖字符混写、同日生产变体、纯数字退化版本、首条统一处置及步数/距离/字数/订单/型号反例。
