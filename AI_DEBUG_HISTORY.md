...
### 6.45 当前健康覆盖历史失败，资源回升覆盖 OOM
- 问题 | 调度累计失败与 OOM 被绿灯覆盖，`failed=False` 被误报且外部 Chromium OOM 被错记到 Mory。根因 | 当前态、历史、压力来源、受害进程和日志子串混用。解法 | 分离一小时阻断/累计值，并拆分全局/cgroup、Mory/外部/未知来源及 Mory 受害。预防 | 回归覆盖布尔反例、来源与受害、截断和证据缺口。
