...
- 根因：生产本轮 AI 不可用后走真实标题兜底，而兜底仍从“讨论/观点”文案池随机取句；此前只调整尾语策略，没有移除新闻卡必须互动的错误职责。
- 解法：删除随机互动池；AI、输出门禁和真实标题兜底统一使用固定时效说明“以上是本次刚刚更新的最新新闻。”；抓取请求增加 no-cache 与时间戳参数。
- 预防：信息卡片只承担信息职责，互动、转化和人设栏目独立设计；生产验收必须覆盖 AI 失败兜底，因为截图问题实际来自该路径。
