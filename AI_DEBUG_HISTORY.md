...
### 后续加固
- 2026-07-06 按老板要求，所有单人禁封管理员通知都必须带可点击解封按钮。
- 已覆盖：统一广告处置通知、编辑消息广告检测通知、全局黑名单入群拦截通知。
- 按钮统一走 `ad_unban:<uid>:<chat_id>` 回调，文案为“一键解封”。
- VPS 备份 `/home/ubuntu/mory_assistant/backups/unban_button_all_ban_notices_20260706_035013`；远端 grep、py_compile、重启、health 均通过。
