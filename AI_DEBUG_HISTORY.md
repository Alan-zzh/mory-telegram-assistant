...
- 问题 | Alembic 重建表前部署器只备份代码，迁移失败或误删数据无法直接恢复。根因 | 代码快照排除数据库，迁移链又未建立在线一致性快照。解法 | 迁移前用 SQLite backup API 生成随机名 0600 快照，校验完整性与外键后才放行。预防 | 备份、校验、迁移保持同一失败关闭链，并保留真实 SHA-256 回执。
