...
- 问题：同日“一日 4oO+ / 9Oo+”为 0 分，后续“长期稳定关系 q裙：数字群号”和“Q裙：数字群号 + 兼职/上门”也为 0 分；联系方式、招聘、彩票、色情和跑分洗钱均可用数字、字母或同音字绕过。
- 根因：旧规范化只处理部分数字与繁体，联系方式词典又只覆盖微信、链接和直白“加群”，没有把 QQ/Q群/qqun 与 q裙/扣郡分级建模；若全局替换 O/I/l、删除插入字符或把“裙/上门”单独定罪，又会误伤英文、型号、服装订单和正常服务。
- 解法：先做 NFKC；收益形近数字只在时间+金额语境转换；明确 QQ 群渠道与 6–12 位数字可作为结构化强证据，歧义 q裙/q郡/扣裙/扣郡必须再有关系招揽、兼职、资源或上门等第二广告锚点；其他六类模板同样分别要求强语义组合。
- 预防：每类规则必须同时具备生产原文、变体、反向语序、首条统一处置和歧义反例；禁止全局 leetspeak 替换，禁止用单个日常词、头像弱特征或普通关系表达直接封禁。
