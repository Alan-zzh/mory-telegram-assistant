...
| 日期 | 类型 | 一句话 | 涉及文件 |
|------|------|--------|----------|
| 2026-09-03 | 修复 | v5.42.24 深夜劝睡不再吞点名与正经提问，补齐积分/签到“作用”问法预设与白名单。 | 消息分发、关键词触发、广告兜底、配置与测试 |
| 2026-09-01 | 修复/治理 | v5.42.23 结构化调度累计失败证据，避免把历史错误误读为当前故障。 | 生产巡检、监控文档与测试 |
| 2026-09-01 | 修复/清理/治理 | v5.42.22 移除17个无效设置，并统一嵌套播报别名的真实配置读写。 | 自然语言、Dashboard、配置兼容、巡检与测试 |
| 2026-09-01 | 修复/安全/治理 | v5.42.21 清理11个退役配置并封死部署、保存、面板和巡检中的复活入口。 | 配置兼容、部署、Dashboard、巡检与测试 |
| 2026-09-01 | 修复/治理 | v5.42.20 区分事务任务窗口与全量调度历史，显示总数和覆盖范围以消除假停滞。 | 生产巡检、部署门禁、监控文档与测试 |
| 2026-09-01 | 修复/稳定性 | v5.42.19 调度巡检认证完整重建回执，拦截伪造、挂起、触发器和回滚失败。 | 调度生命周期、部署门禁与测试 |
| 2026-09-01 | 修复/稳定性 | v5.42.18 投票事务失败可见、过期拒投且踢人中断可恢复，巡检标明历史错误。 | 投票模块、调度巡检与测试 |
| 2026-09-01 | 修复/治理 | v5.42.17 区分正常空Bio与资料查询失败，保留有界复审并消除假降级告警。 | 入群资料复审、测试与版本记录 |
| 2026-09-01 | 清理/运维 | 非保留TokenLab/TokenPass单元已备份并全部屏蔽，防止再次启动。 | 服务器systemd单元与生产回执 |
| 2026-09-01 | 修复/稳定性 | v5.42.16 修正资源证据与转化统计误判，并将启动维护纳入停机总时限。 | 监控、审计、调度生命周期与测试 |
| 2026-09-01 | 清理/运维 | 移除停用TokenLab遗留的4GiB开机交换文件，保留恢复说明并复核三项目在线。 | 服务器fstab、交换文件与审计记录 |
| 2026-09-01 | 修复/安全 | v5.42.15 清理脚本先解析参数，查询帮助或传错参数不再执行服务器写操作。 | VPS清理脚本、测试与版本记录 |
| 2026-09-01 | 修复/治理 | v5.42.14 分离调度当前错误与历史失败，避免已恢复任务继续显示报错。 | 调度API、监控文档与测试 |
| 2026-09-01 | 修复/运维 | v5.42.13 轮转以root读取root看门狗日志，修复午夜logrotate权限失败。 | VPS清理脚本、测试与版本记录 |
| 2026-09-01 | 修复/效率 | v5.42.12 启动广告清理建立逐群可信基线，后续只重试未删除记录。 | 启动清理、数据库与测试 |
| 2026-08-31 | 修复/效率 | v5.42.11 批量核对远端SHA-256，仅上传变化文件；预检断线重连并拒绝清单外路径。 | 部署器、测试与版本记录 |
| 2026-08-31 | 修复/部署 | v5.42.10 固定Linux日志轮转配置使用LF，修复全机logrotate失败并防止复发。 | VPS清理脚本、测试与版本记录 |
| 2026-08-31 | 治理/运维 | 停止并禁用同机非保留项目，保留Mory、MoryFansBot、MediaOps及必要巡检。 | 服务器服务、容器与定时任务 |
| 2026-08-31 | 修复/部署 | v5.42.9 修复Dashboard用绝对脚本路径误判systemd主Bot离线。 | Dashboard状态、测试与版本记录 |
| 2026-08-31 | 修复/安全/治理 | v5.42.8 统一SSH密钥优先回退，关闭sudo假绿，并完成Steel双页限流与访问收敛。 | vps_config、清理脚本、Dashboard、测试与生产治理 |
| 2026-08-31 | 修复/安全 | v5.42.7 修复SSH helper的sudo PTY回显与多行命令引用风险，统一无PTY、POSIX引用和脱敏。 | ssh_helper、测试与版本记录 |
| 2026-08-31 | 修复/治理 | v5.42.6 拆分OOM来源/受害，并治理steel-browser资源上限与真实探针。 | 生产巡检、审计与测试 |
| 2026-08-31 | 修复/治理 | v5.42.5 修复FAQ统计假绿、频道时间戳/回复降级，并让调度历史和cgroup OOM可见。 | FAQ、频道处理、监控与测试 |
| 2026-08-31 | 修复/治理/部署 | v5.42.4 已部署：修复水果机资料广告漏判，并建立代码更新默认生产闭环。 | 资料检测、消息处置、项目规则与测试 |
