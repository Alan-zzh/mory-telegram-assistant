...
def test_deployment_verification_checks_direct_version_truth_and_permissions():
    ...
    assert "SCHEDULER_COLUMNS_MISSING last_status_at" in checks["调度事实"]
    assert "COALESCE(last_status_at,0) >= ?" in checks["调度事实"]
    assert "COALESCE(last_run,0) >= ?" not in checks["调度事实"]
