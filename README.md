...
当前版本 **v5.35.19**：MoryFansBot 与 Mory小助理共用同一套对话原则——先接住当前话题，普通聊天不按固定轮数硬推；价格、内容和权益先去预览，明确购买、看过预览或明确提出定制需求后才进入自助订阅。拒绝、取消和定制概念咨询不推进，近期已给过下单入口时只继续承接细节；私聊零按钮，群聊每轮只有一个与正文一致的入口。模型自行生成的价格、画质、独家权益、定制能力或交付承诺不会直接发给用户；若整段回复都不可信，则按本轮唯一目标使用安全确定性回复。群内纯数字新人验证答案不计聊天、不入画像和历史、不触发 AI。
