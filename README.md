...
当前版本 **v5.38.5**：修复重启后的广告追溯扫描误删正常消息。保护内容群的数据库模式只允许有明确广告证据的当前窗口记录进入删除链，“怎么订阅”等 `score=0 / is_ad=false` 正常消息即使为了连续模式判断被追踪也必须跳过；无证据时不再按消息 ID 范围盲删。启动扫描从数据库只读定位末条消息，不再向群发送并删除“.”探针。
