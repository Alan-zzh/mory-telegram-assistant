...
当前版本 **v5.38.2**：六合彩/特码交易黑话不再落入普通 AI 或等待重复消息，“港澳1-49特码有量”“六彩合/六码名单有量并找庄”等组合首条直接进入统一永久禁言、删除消息和双黑名单处置，普通港澳、名单、农庄及新闻讨论保持放行。早、午、晚三档新闻播报保持下线，三个时段固定为今日黄历、三张塔罗、易经一卦；卡片不再重复显示免责声明。群播内容由本地数据和日期稳定随机引擎生成，跨日变化、同日重试一致，不消耗模型 Token。私聊明确提出风水、塔罗、算卦，或使用 `/fengshui`、`/tarot`、`/iching`、`/风水`、`/塔罗`、`/易经`、`/算卦` 时，也会在进入 LLM 前自动生成零 Token 回复；普通文化讨论继续走正常聊天。可开启每天轮换的单按钮引导，联系 Mory、预览福利与自助订阅各自只出现一次且与正文一致。
