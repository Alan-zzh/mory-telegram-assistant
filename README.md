...
当前版本 **v5.36.0**：两个项目共用 ReplyContract v1。回复者统一为公开的 Mory 小助理，保留清醒、温柔、小傲娇与群聊/私聊差异，只靠措辞和节奏表达，不冒充真人、不写动作小剧场或虚构生活。普通聊天先接话；价格、内容和信任疑问先去预览，明确购买、看过预览或明确定制需求后才进入自助订阅；拒绝、取消和概念咨询不推进，近期 CTA 自动去重。欢迎、新闻、问候、叫醒、召回、购物车、离群和定点播报也遵守相同边界。风格进化只使用管理员审核并启用的安全样本，默认不保存用户/助手原文，AI 不自动发布 Prompt 变化。群内纯数字验证答案继续在统计、历史、画像和 AI 前静默忽略。
