...
当前版本 **v5.36.1**：广告检测补齐繁体“看我賺米”显示名及“1天1w米 / 日1w米”收益黑话，命中后继续走统一永久禁言、删消息、双黑名单处置，同时不把跑步距离和普通微信业务讨论误判为广告。两个项目继续共用 ReplyContract v1：回复者统一为公开的 Mory 小助理，普通聊天无 CTA，了解先预览，明确购买才进入自助订阅，拒绝后停止推进；风格进化只使用管理员审核并启用的安全样本，默认不保存用户/助手原文。
