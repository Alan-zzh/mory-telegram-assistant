...
当前版本 **v5.38.6**：修复“一日 9Oo+ / 4oO+”日收益广告漏判。紧邻加号的 O/o 混写数字会安全规范化为 0，一日/一天/每日/每天加数字加号首条直接进入统一永久禁言、删除消息和双黑名单处置；每天步数、距离、字数、订单统计和型号保持放行。保护内容群的追溯扫描仍只处理有逐条广告证据的当前窗口记录，不会把普通行为追踪当成广告。
