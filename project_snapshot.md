...
## 当前版本
v5.38.42（2026-08-13）· 广告治理、成员资料覆盖分层与项目只读巡检控制面

生产状态：**v5.38.42 协调发布中**。v5.38.41 report-only 枚举覆盖 97.04%，但 Profile 覆盖口径混淆 Bot API 400 而失败闭合，79 候选零处置；最终结果须以新报告、服务版本/哈希、Telegram/数据库读回及三类巡检实跑回执覆盖更新。

## 最近 3 条大事
1. 2026-08-13 v5.38.42：Profile 取证与 Bot API 增强不可用/传输异常分层。
2. 2026-08-13 v5.38.41：资料/消息证据隔离，扫描限速并发并独立门禁覆盖。
3. 2026-08-13 v5.38.39：个人频道、重复刷屏、Q裙广告修复与只读巡检完成。
