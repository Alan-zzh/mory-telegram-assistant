...
生产状态：VPS 双服务 active+enabled，Dashboard health 200/v5.35.18，代码与根目录真相文件 13/13 哈希一致，当前进程错误与强密钥命中均为 0，六层监控 `all normal`。

## 最近 3 条大事
1. 2026-07-25 v5.35.18 销售事实门禁空结果闭环：整段不可信时按唯一目标生成安全回复，不误转人工。
2. 2026-07-25 v5.35.17 生产反例闭环：拒绝短句不再误送预览；模型虚构的价格、画质、独家权益、能力和交付承诺发送前移除。
3. 2026-07-25 v5.35.16 双项目人设与成交链统一：普通聊天不按轮数硬推；了解先预览、明确意向再自助；群承接不再额外轰炸私聊，入口单目标且不虚构服务。
