...
| 配置 / 部署 | 在用 | `core/settings.py`、`deploy_vps.py` + `config.json` | 密钥仅 `.env`；动态发布排除同步冲突副本和内部治理文档，部署前备份、失败保险恢复双服务；v5.38.16 新增 MAX_UPLOAD_FILE_SIZE=20MB + SKIP_PATH_FRAGMENTS 黑名单（runtime/cache/logs/audit-reports/demo/__pycache__/.git）；v5.38.17 补 SKIP_PATH_FRAGMENTS=_quarantine_/EXECUTION_/_tmp_ 路径片段兜底 + EXCLUDE_NAMES=EXECUTION_LOG.md/EXECUTION_REPORT.md 根目录过程流水精确排除；v5.38.18 ALLOWED_CONFIG_FIELDS 补 AI_REQUEST_TIMEOUT/AI_MAX_ATTEMPTS，三处同步闭环；v5.38.19 bot_initializer/config_repo 两处动态/系统状态日志明文脱敏 + daily_report 回退留痕闭环；v5.38.20 faq_api 11 端点/health_api 6 处嵌套 JSON detail/audit 缺失键样本 PII 泄露全治 + weekly/monthly 2 处周报月报群人数回退裸 except 清场 + health_api 7 处 except 留痕闭环 + ALLOWED_CONFIG_FIELDS 补 BROADCAST_IMAGE_CARD_ENABLED/BROADCAST_THEME_ENABLED 两处播报图片卡三处同步第三处差集 |
