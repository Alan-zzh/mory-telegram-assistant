...
## 当前版本
v5.38.63（2026-08-17）· 关键词延迟删多规则独立计时与管理员双端配置

生产状态：**v5.38.62 已部署；v5.38.63 已完成本地 1179 项单元测试，生产部署与现存匹配消息清理待本次发布闭环。**

## 最近 3 条大事
1. 2026-08-17 v5.38.63：关键词规则可独立计时，由双端管理员管理并清理现存消息。
2. 2026-08-17 v5.38.62：预填关键词群消息延迟删除，重启恢复且不处罚用户。
3. 2026-08-17 v5.38.61：只保留当前10个型号并按到期优先；临时故障回切，仅额度耗尽拉黑。
