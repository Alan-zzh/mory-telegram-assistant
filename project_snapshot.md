...
生产状态：**部署前只读基线为 v5.42.4：双服务 active、health 200、数据库完整；最新审计已排除 `failed=False` 日志假报警，但一小时内 cgroup OOM 68 次且 load1 7.44，来源为同机 Chromium 容器，v5.42.5 仍须等待资源发布门禁恢复并重新取证。**
