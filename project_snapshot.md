...
## 当前版本
v5.38.40（2026-08-13）· 全量成员广告扫描证据隔离

生产状态：**v5.38.39 已部署，首轮只报告扫描因普通 username 跨字段误判已熔断，零治理副作用；v5.38.40 待门禁与发布**。

## 最近 3 条大事
1. 2026-08-13 v5.38.40：资料和逐条消息证据隔离，普通 username 反例进入门禁。
2. 2026-08-13 v5.38.39：存量扫描与实时广告规则统一，报告和处置分离。
3. 2026-08-12 v5.38.38：部署日志过滤与生产依赖锁读回完成修复。
