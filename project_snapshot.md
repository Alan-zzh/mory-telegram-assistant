...
## 当前版本
v5.42.15（2026-09-01）· 已部署：调度错误范围纠正与清理CLI参数安全门

生产状态：**Mory v5.42.15双服务active/enabled、NRestarts=0、health 200，数据库与17项生产巡检通过；4条累计失败均标记historical，非当前last_error为0；清理CLI的help为零服务器副作用。MoryFansBot与MediaOps-COO在线，同机非保留项目保持停用。**

## 最近 3 条大事
1. 2026-09-01 v5.42.15已部署：清理脚本参数错误不再执行写操作。
2. 2026-09-01 v5.42.14已部署：调度当前错误与历史失败分开呈现。
3. 2026-09-01 v5.42.13已部署：修复root看门狗日志轮转权限。
