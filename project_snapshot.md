...
## 当前版本
v5.38.38（2026-08-12）· 全链路真相治理已发布

生产状态：**v5.38.38 已部署**。2026-08-12 23:27 CST 双服务以新 PID 启动且 active、NRestarts=0，health 200；406/406 文件哈希零差异，Dashboard 运行时精确匹配 lock，数据库与权限读回通过。

## 最近 3 条大事
1. 2026-08-12 v5.38.38：部署日志只过滤确证退出栈，生产 Dashboard 依赖按 lock 读回。
2. 2026-08-12 v5.38.37：自助复权、假绿灯、热重载/停机竞态与部署权限完成修复。
3. 2026-08-12：删除无入口的新闻配置面和空报表模块，能力矩阵改为真相索引。
