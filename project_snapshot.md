...
## 当前版本
v5.38.41（2026-08-13）· 广告治理、限速成员扫描与项目只读巡检控制面

生产状态：**v5.38.41 协调发布中**。首轮证据误判和顺序扫描吞吐不足均已在 report-only 阶段熔断，零治理副作用；最终结果须以成员扫描报告、服务版本/哈希、Telegram/数据库读回及三类巡检实跑回执覆盖更新。

## 最近 3 条大事
1. 2026-08-13 v5.38.41：资料/消息证据隔离，扫描限速并发并独立门禁多维覆盖。
2. 2026-08-13 v5.38.39：个人频道、重复刷屏、Q裙广告修复与只读巡检控制面完成。
