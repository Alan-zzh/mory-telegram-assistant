...
## 当前版本
v5.38.41（2026-08-13）· 全量成员扫描限速并发与多维覆盖门禁

生产状态：**v5.38.40 已部署；首轮证据误判和顺序扫描吞吐不足均已在 report-only 阶段熔断，零治理副作用；v5.38.41 待门禁与发布**。

## 最近 3 条大事
1. 2026-08-13 v5.38.41：扫描限速有界并发，资料与个人频道覆盖率独立失败闭合。
2. 2026-08-13 v5.38.40：资料和逐条消息证据隔离，普通 username 反例进入门禁。
3. 2026-08-13 v5.38.39：存量扫描与实时广告规则统一，报告和处置分离。
