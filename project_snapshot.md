...
生产状态：**当前仍为 v5.42.4：双服务 active、NRestarts=0、health 200、数据库完整。共享主机 OOM 已确认来自 `steel-browser/Chromium`；11:01 将其从 1GiB RAM+1GiB 额外 swap 调整为 1.5GiB RAM+无额外 swap 并受控重启，当前 cgroup OOM/swap 均为 0、真实 Steel 业务探针 7/7，通过一小时观察窗后再发布 v5.42.6。**

## 最近 3 条大事
1. 2026-08-31 v5.42.6：OOM 准确归因并治理 Steel 资源上限，等待观察窗放行。
