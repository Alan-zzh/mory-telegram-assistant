...
## 当前版本
v5.42.7（2026-08-31）· 待部署：SSH helper 凭据回显/命令引用风险与生产巡检真实性修复

生产状态：**当前仍为 v5.42.4：双服务 active、NRestarts=0、health 200、数据库完整。共享主机 OOM 来自 `steel-browser/Chromium`；2GiB 方案于 14:18 再次被 5 个页面真实负载证伪，14:27 仅重启 Steel 后固定探针 7/7、短样本零新事件。raw CDP 页面容量和合法入口尚未确认，Mory 发布继续阻断。**

## 最近 3 条大事
1. 2026-08-31 v5.42.7：修复 SSH root 执行的 PTY 回显与多行命令引用风险。
2. 2026-08-31 v5.42.6：OOM 准确归因，Steel 2GiB 真实门禁失败。
3. 2026-08-31 v5.42.5：统计、频道和巡检真实性修复已完成，待部署。
