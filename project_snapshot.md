...
## 当前版本
v5.42.7（2026-08-31）· 待部署：SSH helper 凭据回显风险与生产巡检真实性修复

生产状态：**当前仍为 v5.42.4：双服务 active、NRestarts=0、health 200、数据库完整。共享主机 OOM 来自 `steel-browser/Chromium`；1.5GiB 与 2GiB 方案均被真实负载证伪，事故后 Steel 已受控恢复、固定探针 7/7，但 raw CDP 页面容量和合法入口尚未确认，Mory 发布继续阻断。**

## 最近 3 条大事
1. 2026-08-31 v5.42.7：修复 SSH root 执行的 PTY 密码回显风险。
