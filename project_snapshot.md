...
> 本文件每次整段覆盖对应区块，禁止无限追加。最后更新：2026-08-24。
...
| 广告检测 | 在用 | `modules/ad_detector.py`、`modules/ad_patterns_encoded.py`、`modules/ad_profile_signals.py`、`modules/ad_enforcement.py`、`modules/member_ad_scan.py`、`core/handlers/*` | 歧义联系方式仅作弱信号；设备挂机收益组合及私密群 Bio 三锚点招揽直接处置，明确反诈与正常电脑讨论放行；处置事件保留首次根因，24小时说明卡支持本人限频复检，高风险/未知状态拒绝自动恢复 |
...
## 当前版本
v5.41.1（2026-08-24）· 设备挂机收益广告与私密群 Bio 招揽变体热修

生产状态：**v5.41.1 已于 2026-08-24 部署：339/339 文件上传，双服务 active/NRestarts=0、health 200，广告检测关键哈希一致；20:19 消息 69923 已删除并标记广告，UID 8993979428 已限制发言且双黑名单/禁言记录持久化。启动窗无真实 ERROR/CRITICAL，journal 凭据扫描 0 命中。**

## 最近 3 条大事
1. 2026-08-24 v5.41.1：设备挂机收益广告与私密群 Bio 招揽热修已部署，历史广告已补删并限制账号。
2. 2026-08-23 v5.41.0：全仓治理——凭据搬家、原子写、发送重试、假监控拆除、卫生闸门。
3. 2026-08-23 v5.40.0：三栏目内容丰富——易经经典句、黄历值神/吉时、塔罗框架轮换、免责尾注、敏感分流，均默认关闭。
