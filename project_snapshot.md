...
## 当前版本
v5.38.40（2026-08-13）· 实时/存量广告治理证据隔离与项目只读巡检控制面

生产状态：**v5.38.40 协调发布中**。首轮只报告扫描因普通 username 跨字段误判已在处置前熔断，零治理副作用；最终结果须以成员扫描报告、服务版本/哈希、Telegram/数据库读回及三类巡检实跑回执覆盖更新。

## 最近 3 条大事
1. 2026-08-13 v5.38.40：资料/消息证据隔离，广告漏判、存量扫描和只读巡检合并到同一基线。
2. 2026-08-13 v5.38.39：个人频道、重复刷屏和Q裙广告修复完成。
