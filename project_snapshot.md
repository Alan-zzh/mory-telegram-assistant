...
## 当前版本
v5.42.5（2026-08-31）· 待部署：统计、频道与生产巡检真实性修复

生产状态：**部署前只读基线为 v5.42.4：双服务 active、health 200、数据库完整；最新审计已排除 `failed=False` 日志假报警，load1 已短时回落到 4.10，但一小时内 cgroup OOM 仍达 69 次且来源为同机 Chromium 容器，v5.42.5 仍须等待资源发布门禁恢复并重新取证。**

## 最近 3 条大事
1. 2026-08-31 v5.42.5：统计、频道和巡检真实性修复已完成，待资源门禁放行。
2. 2026-08-31 v5.42.4：水果机分销资料广告组合已部署，生产正反例通过。
3. 2026-08-31 v5.42.3：预设白名单与外部事项隔离已部署，生产回放通过。
