...
## 当前版本
v5.42.19（2026-09-01）· 已部署：调度巡检认证完整重建回执，拦截伪造、挂起、场景触发器与回滚失败

生产状态：**Mory v5.42.19双服务active/enabled、NRestarts=0、health 200，关键哈希一致、数据库完整、17项生产巡检通过；认证调度回执为startup generation=1、managed_jobs=41。MoryFansBot与MediaOps-COO在线；非保留进程为0，TokenLab/TokenPass均masked，Docker/containerd均inactive。**

## 最近 3 条大事
1. 2026-09-01 v5.42.19已部署，调度巡检改用来源认证的完整重建回执。
2. 2026-09-01 v5.42.18已部署，投票事务恢复与持久复核通过。
3. 2026-09-01 v5.42.17已部署，空Bio与查询失败分流通过。
