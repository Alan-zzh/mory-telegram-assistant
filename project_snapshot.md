...
| 广告检测 | 在用 | `modules/ad_detector.py`、`modules/ad_profile_signals.py`、`modules/ad_enforcement.py`、`modules/member_ad_scan.py`、`core/handlers/*` | 实时与存量扫描共用显示名/username/Bio/Premium/个人频道/历史消息/高置信头像规则；全量扫描默认只报告，覆盖率不足、管理员/白名单或状态未知均不处罚，应用时重新取证并走统一处置链 |
...
## 当前版本
v5.38.42（2026-08-13）· 全量成员资料覆盖与 Telegram 增强能力分层

生产状态：**v5.38.41 已部署；全量 report-only 枚举覆盖 97.04%，但 Profile 覆盖口径混淆 Bot API 400 而失败闭合，零治理副作用；v5.38.42 待门禁与发布**。

## 最近 3 条大事
1. 2026-08-13 v5.38.42：Profile 取证与 Bot API 增强不可用/传输异常分层。
2. 2026-08-13 v5.38.41：扫描限速有界并发，资料与个人频道覆盖率独立门禁。
3. 2026-08-13 v5.38.40：资料和逐条消息证据隔离，普通 username 反例进入门禁。
