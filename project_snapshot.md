...
## 当前版本
v5.42.20（2026-09-01）· 待部署：区分事务任务窗口与全部调度历史，固定显示历史总数与覆盖范围

生产状态：**Mory v5.42.19双服务active/enabled、NRestarts=0、health 200，关键哈希一致、数据库完整、17项生产巡检通过；生产 task_execution_history 共209行，最近1小时0行仅表示该窗口无事务任务。MoryFansBot与MediaOps-COO在线；非保留进程为0，TokenLab/TokenPass均masked，Docker/containerd均inactive。**

## 最近 3 条大事
1. 2026-09-01 v5.42.20待部署，巡检区分事务窗口与全部调度历史。
