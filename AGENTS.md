...
> 规则版本 v5.39.1（2026-08-31）| 本文件只保存长期决策与红线；技术步骤指向 `docs/technical/`，运行态结论必须重新取证。
...
### 代码更新默认生产闭环（2026-08-31 老板长期授权）
- 老板要求修改代码、配置或运行行为时，默认授权在实现与验证完成后，同一任务内完成补丁升版、必要记录、Git commit、既定生产环境部署、双服务重启和部署后稳定性验收；不再为上述标准闭环重复询问发布授权。代码未部署、只有本地测试或只看 health 200 均不得报完成。
- 自动发布只适用于既定 `/home/ubuntu/mory_assistant` 生产目标和现有安全部署器；不包含 `git push`、破坏性数据库/数据治理、凭据轮换、真实账号处置、付费动作、换服务器或新增外部目标，这些仍须单独明确授权。
- 纯问答、只读诊断、计划、实验稿、文档说明以及门禁失败的改动不部署。发布前必须工作树无未提交项、版本与记录一致、相关测试和全仓 unit 通过；发布后必须读回 VPS 版本/哈希、双服务状态与重启计数、health、启动窗口日志、数据库完整性及受影响业务探针。任一项失败即停止发布声明并进入受控回滚或门禁阻断。
