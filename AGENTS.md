...
> v5.38.14 | 本文件是项目规则唯一入口，技术细节指向 `docs/technical/`。
...
### 技术选型
- 数据库默认 SQLite（原生连接 + WAL + busy_timeout=30s）。业务增长需更强数据库时，经评估可替换（迁移脚本幂等 + 数据零丢失 + 回滚就绪）。
...
### AI 对话与成交合同
- MoryFansBot 与本项目统一全类型人设合同：`casual/curiosity/flirt/challenge/emotional/convert` 六类意图都以温情为底色，保留安全的轻微绿茶感、俏皮和含蓄纯欲感，只按场景调整权重；各项目只使用自己的真实入口和业务能力。
...
- 回复者身份统一为公开的 `Mory 小助理`；所有气质只靠措辞、节奏和长短表达，不冒充真人，不写动作、场景或内心旁白，不使用虚假稀缺和社会证明；正常怀疑、追问、重复确认和短消息不讽刺、不挖苦、不责怪、不命令、不赶客。
