...
# 发送触发消息获取起始点
print("\n📨 发送触发消息...")
cmd = 'curl -s -X POST "https://api.telegram.org/bot' + BOT_TOKEN + '/sendMessage" -d \'{"chat_id":' + GROUP_ID + ',"text":"🔍 开始扫描..."}\''
...
ssh.exec_command('cat > /tmp/scan3.py << "SCRIPT"\n' + scan_script + '\nSCRIPT')
stdin, stdout, stderr = ssh.exec_command('python3 /tmp/scan3.py', timeout=180)
out = stdout.read().decode('utf-8', errors='replace').strip()
print(out)

# 删除触发消息
print("\n🗑️ 删除触发消息...")
cmd = 'curl -s -X POST "https://api.telegram.org/bot' + BOT_TOKEN + '/deleteMessage" -d \'{"chat_id":' + GROUP_ID + ',"message_id":' + str(trigger_msg_id) + '}\''
stdin, stdout, stderr = ssh.exec_command(cmd)
