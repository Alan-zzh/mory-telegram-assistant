...
1. **[v4.0] 架构级除虫与重构（3项）**
2. [v21.47] 隐藏地雷修复（3项）
3. [v21.46] 终极代码审查修复（5项）
4. [v21.45] 竞态探测刷屏Bug
5. [v21.44] Bot 409 Conflict冲突
6. [v21.44] 阅后即焚追踪污染问题
7. [v21.43] 新闻播报发两条 + 背刺泄密频率过高
8. [v21.42] 阅后即焚两大功能失效
9. [历史] Bot状态显示"已停止"
...
## [v4.0] Bug #9：架构级除虫与重构（3项）

> 最后更新：2026-04-19

### Bug #9.1：斩断死锁 - 废除 forward_message 探测

**问题描述**：
- 愚蠢的竞态探测逻辑导致API滥用
- `bot.forward_message(admin_id, ...)` 会触发Telegram流控
- 异步延迟导致追踪状态错乱

**修复方案**：改为"只管发，只管存"，删除交给后台定时任务。
...
**问题描述**：用户回复了机器人，但系统无法识别，数据库 replied 永远是 0。

**修复方案**：新增全局 handler，优先捕获回复事件，秒级更新数据库。
...
**问题描述**：强依赖 Linux 字体路径，Windows/新VPS上崩溃。

**修复方案**：添加 arial.ttf 兜底，强制使用默认字体。

**涉及文件**：`modules/content.py`

---

*文档版本：v4.0 | 最后更新：2026-04-19*
