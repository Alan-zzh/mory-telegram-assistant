...
- “怎么订阅 / 如何订阅 / 订阅入口或链接 / 想订阅 / 明确开通”等强意图必须绕过 P7.5 主动搭讪旁路，进入 P10 统一成交发送。正文结合近期预览语境做清醒、温柔、略带小傲娇的承接；群聊只挂一个 `@MorychannelBot` 自助下单按钮，私聊只给正文入口。
- 明确成交短句按“群聊/私聊 × 有无近期预览”分池选择，每次优先排除最近 6 条助手历史已经发送过的完整原句；池耗尽后也不连续重复上一句。随机只改变短句措辞，不改变目标、入口、按钮数量或事实边界。
- 入口识别覆盖订阅、开通、会员、下单、付费、付款及繁体近义表达，包括“咋订阅、订阅怎么弄、会员怎么开、开通一下、付费入口、訂閱連結”等；普通商品“咖啡怎么买/鞋子怎么购买”没有 Mory 业务上下文时仍为 `none`。
- P7.5 只有真实消息发送成功才允许返回已处理；发送对象缺失或发送失败必须返回 `False` 继续 P10，禁止写成功日志后静默吞掉成交回复。
