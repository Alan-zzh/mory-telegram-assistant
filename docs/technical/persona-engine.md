# Mory 人设引擎 v5.35.10 — 技术文档

> **被 [AGENTS.md](../../AGENTS.md) 索引引用 · 适用版本：v5.35.10+**
> **最后更新**：2026-07-25（v5.35.10 正常聊天去舞台化）
...
### 4.3 近期对话承接与定制转化（v5.35.15）

- 当前轮从 `conversation_telemetry` 读取同一 `user_id + chat_id` 最近30分钟的3轮问答，最多向模型传6条 `user/assistant` 消息；跨群、跨私聊和过期内容不进入当前上下文。
- `IntentRouter` 与 P9 mode 使用同一上下文购买判断。“定制舞”是直接购买锚点，后续“就是这个味”“喜欢这种风格”“卡点变装”等承接短句继续保持 `purchase_intent/convert`。
- 明确拒绝、普通闲聊及无购买锚点的泛化风格表达不升级；生产旧 `CONVERT_KEYWORDS` 覆盖也不能删除“定制/定做/专属定制”三个必选项。
- 明确定制首次命中时可在正文给一次 `@MorychannelBot` 自助下单入口；最近 6 条助手历史已经给过下单入口时，后续确认和细节补充仍保持购买阶段，但只承接内容，不重复链接或按钮。用户明确再次询问怎么买、下单链接或下单入口时才允许重发。
- 私聊销售回复只用正文可点击入口，不挂 inline button；非私聊也只能生成一个与当前目标一致的按钮。泛问“群/链接/入口”默认只给预览，明确索要下单入口才给订单入口；未解决问题只给人工入口，预览、下单和人工入口不在同一轮混用。
