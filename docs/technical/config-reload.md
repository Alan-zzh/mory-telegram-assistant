...
> 本文档为独立技术说明，未被 AGENTS.md 直接索引 · 适用版本：v5.10.2+
...
| 5 秒 | 延迟低 | 仍会造成无意义轮询 | ❌ |
| **30 秒** | **低占用，Dashboard 已通过 reload_flag 显式唤醒** | 最坏等待 30 秒 | ✅ **采用** |
| inotify | 实时 | 部署复杂 | ❌ |

#### 5.2 为什么不直接 reload 全部模块？

- **当前方案**：原子替换运行时配置字典，并由 `bot_initializer` 显式重编排 scheduler 与 scene jobs。
- **边界**：纯读取 `config` 的模块下一次调用即可见；持有独立缓存的模块仍需注册自己的刷新动作。
- **门禁**：新增调度配置必须验证 reload 前后当前注册集，而不是只看 `reload_flag` 被消费。

#### 5.3 失败兜底

- reload_flag 文件丢失（人为删除 / 系统清理）→ Bot 不会重载，但**不影响 Bot 正常运行**（仍是旧配置）
- 用户再次改 Dashboard → 重新创建 flag → 下一轮轮询消费
- **最坏情况**：30 秒轮询延迟；重载失败保留旧配置并记录错误，不用半更新状态冒充成功
