...
最后更新：2026-07-24（v5.35.8 新闻 10 条综合头条与轮询韧性）
...
- 首选源不足 10 条时不会立刻发送残缺卡片，而是继续后备源；两个来源都不足时按标题去重合并。候选充足但类目集中时逐级放宽类目/单源软限制，科技和财经硬上限不放宽。
- AI 严格输出 10 条正文，第 11 行才是观察；输出条数不合格时不用残缺文案，直接回退 10 条真实标题。
- `PROMPT_TEMPLATES` 中不含“严格只写10条 + 第11行”的新闻旧覆盖会被自动忽略，避免生产遗留 5 条模板重新覆盖代码契约；符合新契约的自定义模板仍可使用。
- HTML 使用 10 个 `📌`，Rich Message 使用 10 个 `<li>`；内部类目、来源名和聚合方式只参与选题与日志，不出现在用户消息。
