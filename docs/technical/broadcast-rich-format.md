...
最后更新：2026-07-27（v5.37.0 风水塔罗栏目替换新闻）
...
- 默认栏目为 09:05 风水宜忌、13:05 塔罗播报、20:35 晚间宜忌；`MYSTIC_BROADCAST_CONFIG` 可分别调整时刻和栏目类型，新能力缺少显式 `enabled=true` 时保持关闭。
- 正文使用面向全群的公共信息结构，禁止出现“给你的问题”、对个人情绪/内心的判断或心理教练式要求。
- 内容使用北京时间日期、时段和栏目类型生成稳定 seed：跨日变化，同一天重试不换内容，避免失败重试出现不同牌面。
- 风水内容只给空间整理、颜色和自我提醒；塔罗只给反思角度与可执行小动作；不把预测写成事实，不要求购买“转运”物品。
- HTML 与 Rich Message 都固定显示娱乐边界和 `@MoryMateBot` 署名；正文门禁拒绝新闻、热搜、链接、下单、订阅和私聊入口。
