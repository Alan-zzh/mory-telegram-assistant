...
最后更新：2026-07-27（v5.38.1 三时段传统文化栏目）
...
- v5.35.13：第 6 行不再总结新闻，改为随机的人设互动尾语；温情自白、邀聊、价值表达和定制沟通四类策略轮换。
- v5.36.2：第 6 行取消随机互动与观点，固定显示“以上是本次刚刚更新的最新新闻。”；每轮抓取附带 no-cache 与时间戳参数。
- v5.37.0：三档新闻调度整体退役，原时段替换为风水小签、塔罗牌和晚间能量签；同日稳定随机，不使用新闻源或 LLM，不点名群友、不挂销售入口。
- v5.37.1：玄学卡片改为群公共栏目；删除个人心理提问、情绪判断和说教式动作，只保留宜忌、方位、参考色、牌面、关键词及明日准备等短信息。
- v5.38.1：三档卡片删除重复免责声明；私聊明确风水、塔罗、算卦请求可在 LLM 前由本地引擎自动回复，零模型 Token，普通讨论不抢答。
- v5.38.0：三个时段升级为固定且完全不同的产品：真实今日黄历、三张塔罗、易经一卦；Rich/HTML 使用分区排版，支持按日轮换的单 CTA。
...
- 生产主动触达只启用 09:05 黄历、13:05 塔罗、20:35 易经；早午晚泛问候和四个定点播报关闭，避免同一时段重复刷屏。
- 泛问候/定点播报代码仅作为可控能力保留；重新启用必须先复核与三档传统文化栏目的时间间隔和内容差异。
...
- 问候 AI 失败或输出不合格时本轮不发送，禁止固定套话兜底；定点播报若人工开启，才使用各自静态正文兜底。
- 问候图片卡只有一份经质量门禁的正文，不再拼接独立随机的“今日一句”或“一言”。
...
### v5.35.9 新闻头条契约【已下线 v5.37.0】历史契约保留供参考
...
- 局部 `MODE_ROUTING` 只覆盖明确配置项；实时问候跳过 code 模型和仅思考模型，模型不可用时跳过发送，不冒充成功。

### v5.35.13 新闻人设互动尾语契约【已下线 v5.37.0】历史契约保留供参考

- 前 5 条只负责交代新闻事实；第 6 行只负责建立小助理人设和引导互动，禁止总结、概括、点名或复述前 5 条新闻。
- 第 6 行必须使用第一人称，随机采用四类策略之一：温情自白、邀请群友来聊、表达想成为怎样的小助理、引导有想法的人联系 Mory 沟通定制。
- 输出门禁要求尾语包含人设/互动意图，并拒绝与新闻事实共享具体短语；旧的“事实锚定观察”提示词会被识别为过期配置并自动忽略。
- LLM 输出条数或尾语策略不合格时，前 5 条继续使用真实标题，尾语从四类人工约束策略池随机选择，不再回退新闻摘要。
- HTML 的 `<blockquote>` 与 Rich Message 的 `<blockquote>` 继续承载第 6 行，但其业务含义从“新闻观察”变为“人设互动尾语”。

### v5.36.2 新闻固定时效说明契约【已下线 v5.37.0】历史契约保留供参考

- 前 5 条继续从本轮实时抓取的 10 条综合候选中筛选；第 6 行不再承担互动、人设、观点或转化职责。
- AI 提示词、输出门禁和真实标题兜底只接受固定句“以上是本次刚刚更新的最新新闻。”，截图中的“不同想法/群里说说”等文案直接判为不合格。
- NewsNow 与直连新闻请求发送 `Cache-Control: no-cache`、`Pragma: no-cache`，并附带当前时间戳查询参数，降低中间缓存复用上一轮榜单的概率。
- 上游榜单不提供统一发布时间字段，因此“最新”指本轮播报时重新请求到的当前榜单，不虚构单条新闻的发布时间。

### v5.38.0 三时段传统文化栏目契约

- 新闻抓取、提示词、格式化与调度执行链均已删除；真实调度键只有 `mystic_morning`、`mystic_afternoon`、`mystic_evening`。
- 默认栏目身份固定：09:05 今日黄历、13:05 三张塔罗、20:35 易经一卦；Dashboard 只调整时间与 CTA 开关，不允许把三个时段重新配置成同一种栏目。
- 早间通过锁定的 `cnlunar==0.2.4` 读取真实农历、年月日时干支、宜忌、冲煞、十二值日、二十八星宿、吉神方位、节气和彭祖百忌；依赖缺失时任务失败并重试，不用编造字段兜底。
- 午间从内部策展的 22 张大阿卡纳中无重复抽取主牌、助力和提醒，分别计算正逆位关键词，再生成主导元素、今日主轴和组合观察。
- 晚间使用完整文王六十四卦映射；选出本卦后只翻转一个真实动爻，由六爻结果反查之卦，并展示上下卦、动爻标签和变化方向。
- 内容使用北京时间日期、时段和栏目类型生成稳定 seed：跨日变化，同一天重试不换内容或 CTA，避免失败重试前后矛盾。
- Rich Message 采用 `h2 + kicker + meta blockquote + 多个 h3 分区 + insight blockquote + footer`；HTML 降级路径保留相同信息层级，不重复附加免责声明。
- `cta_enabled` 默认关闭。开启后，联系 Mory、预览福利、自助订阅三个目标每天洗牌并一一分配给早午晚三档；每卡严格一个按钮，正文 closing、按钮文字和 URL 必须同一目标。
- 正文不放 URL 或机器人入口；`@MoryMateBot` 只作发送者署名。联系 Mory 使用 `Moryfansbot`，预览使用 `moryselect`，自助订阅使用 `MorychannelBot`。
- 内容层继续使用受约束的数据源与措辞，不把推演写成确定事实，也不要求购买“转运”物品；这些边界由代码和测试保证，不机械复制到每张卡片。
- 旧 15:00 定向“哥哥～”塔罗只有 `legacy_targeted_tarot_enabled=true` 才注册，默认关闭，避免点名打扰或与三档栏目重复。
- `private_reply_enabled` 默认关闭。开启后，仅私聊中的明确请求或 `/fengshui`、`/tarot`、`/iching`、`/风水`、`/塔罗`、`/易经`、`/算卦` 命令进入本地引擎；同一用户同日同主题结果一致，跨日或换主题变化，回复遥测标记 `local_zero_token`，普通文化讨论继续走正常聊天。
...
- 旧逻辑：AI 超时后从固定话术池抽取，连续多天容易出现重复早安；问候卡还会叠加两段无关随机文案。
- 新逻辑：模型失败或质量不合格时本轮跳过；图片只展示本轮正文；生产关闭泛问候，主动触达收敛为三档内容栏目。
