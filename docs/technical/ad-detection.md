...
资料层链接与频道边界：Bio 中只有 URL、`t.me`/`telegram.me` 地址，或账号只绑定了一个普通个人频道，均不构成广告证据。必须在同一字段或关联频道内容中再命中明确广告关键词、招募交易、收益承诺、色情或灰产组合语义，才允许进入统一处置链；群邀请链接与“多一条路试试/看看”等规避式引流话术落在同一 Bio 时属于高置信组合。姓名、username、Bio 默认不得跨字段拼接；允许的窄范围跨字段强证据包括“老师/同城/同程+免费上榜”姓名配 Bot 拉新深链，以及“水果+14–20 型号+手机全系”暗语姓名或“规模词+代理/散户+出货/预定”资料配 Bio 邀请链接/关联频道。后一类在群里继续发送“寻手机店/渠道商合作拿货”等交易续句时，正文也按逐条广告处理。普通姓名、普通群邀请、真实水果种植/礼盒、摄影交流、维修教程和裸个人链接仍不得授权处置。
...
歧义联系方式边界（v5.38.68）：裸 `t.me`、普通 `@username`、支付宝、客服、加群、加我、点我、起飞及普通私信表达最多是弱信号，多个弱信号不能互相叠加到永久封禁阈值。只有明确账号组合，或与收益、招募、成人、灰产等独立强证据同时出现时才进入永久处置。

### 二点二、处置说明卡与本人复检（v5.38.70）

- `ad_enforcement_events` 保存脱敏规则 ID、类别、字段、强弱、根事件、处置与恢复结果；不保存完整 Bio 或私人消息原文。
- P1 黑名单重申只关联根事件，不覆盖首次原因和时间；`high` 确证广告静默处置，不发送群卡。仅 `low` / `ambiguous` 疑似误判事件可播报，且同一群 24 小时只保留一张共享说明卡。
- 共享卡提供“已整改，本人复检解封”和联系管理员按钮。点击后按点击者 UID 与卡片所在群定位其根事件；无本群待复检记录不会改变任何状态。本人复检仍为 60 秒限频、最多 5 次、每个事件最多成功恢复一次。
- 仅低风险/歧义根事件在当前昵称、用户名、Bio、关联频道、CAS/联邦状态和其他未解决高风险事件全部干净时自动恢复；成人、灰产、收益/招募、明确联系方式、外部黑名单或未知状态保持禁言。
- 自动恢复统一调用 `restore_ad_user()`，清空本地黑名单、全局黑名单、禁言记录、可疑追踪并读回 Telegram 权限；历史已删消息不可恢复。
...
- “同城 PC”只有再同时出现人工审核/平台担保交易与拒绝被骗/PC 无忧两组背书时才作为成人交易直证；普通 PC 维修、装机、审核作业和反诈讨论继续放行。
- `SM` 不再作为裸关键词定罪：必须是独立缩写且邻近交友、调教、全套、特服等明确成人招揽语义；`Smith`、`Smart`、`Smile`、`Cosmos` 等英文姓名或单词中的 `sm` 子串强制放行。
...
- 第一阶段按限速有界并发同时读取 MTProto 完整资料与 Bot API 增强字段，仅写带版本、时效、多维覆盖率和 SHA-256 指纹的私有报告；成员枚举、Profile 取证、资料评估或真实传输覆盖不足时报告失败且不可应用。
- Bot API 对未建立私聊的成员返回 400 时单列为 `personal_chat` 增强不可用，不冒充 Profile 或并发失败；429/5xx/网络异常仍按 transport coverage 失败闭合。个人频道帖子不可读只能降低附加内容覆盖，不能凭空产生广告证据。
- 第二阶段先重新枚举成员建立当前 peer cache，再逐候选查询成员、管理员/白名单/黑名单、个人资料及关联频道证据；成员/Profile、真实传输或个人频道帖子状态未知一律跳过。
