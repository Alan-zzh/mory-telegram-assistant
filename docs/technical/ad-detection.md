...
资料层链接与频道边界：Bio 中只有 URL、`t.me`/`telegram.me` 地址，或账号只绑定了一个普通个人频道，均不构成广告证据。必须在同一字段或关联频道内容中再命中明确广告关键词、招募交易、收益承诺、色情或灰产组合语义，才允许进入统一处置链；群邀请链接与“多一条路试试/看看”等规避式引流话术落在同一 Bio 时属于高置信组合。姓名、username、Bio 不得跨字段拼接广告证据。
