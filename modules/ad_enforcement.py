# -*- coding: utf-8 -*-
"""
[Codex] 广告账号统一处置：不踢人，只永久禁言、删消息、双黑名单。
"""
...
def _safe_delete(bot, db, chat_id: int, msg_id: int) -> bool:
    ...
    try:
        ...
        err = str(e).lower()
        if "not found" in err:
            logger.debug(f"[Codex] 广告消息已不存在: chat={chat_id} msg={msg_id}")
        else:
            logger.debug(f"[Codex] 删除广告消息失败: chat={chat_id} msg={msg_id} err={e}")
    try:
        if db and hasattr(db, "mark_message_deleted"):
            db.mark_message_deleted(chat_id, msg_id)
    except Exception as e:
        logger.debug(f"[Codex] 标记广告消息删除失败: chat={chat_id} msg={msg_id} err={e}")
    return deleted
...
def _mute_forever(bot, chat_id: int, uid: int) -> bool:
    ...
    try:
        ...
        logger.warning(f"[Codex] 永久禁言广告账号失败: chat={chat_id} uid={uid} err={e}")
        return False
...
def _write_blacklists(bot, db, uid: int, reason: str) -> bool:
    ...
    try:
        ...
        ok = False
        logger.warning(f"[Codex] 写入global_blacklist失败: uid={uid} err={e}")
    try:
        if hasattr(db, "blacklist_add"):
            db.blacklist_add(uid, reason)
    except Exception as e:
        ok = False
        logger.warning(f"[Codex] 写入blacklist失败: uid={uid} err={e}")
    return ok
...
def _cleanup_user_messages(bot, db, config: dict, uid: int, chat_id: int, current_msg_id: int = 0) -> int:
    ...
    try:
        messages = db.get_user_messages(uid, chat_id, limit=500) if hasattr(db, "get_user_messages") else []
    except Exception as e:
        logger.debug(f"[Codex] 查询广告用户历史消息失败: uid={uid} err={e}")
        messages = []
    for item in messages:
        try:
            mid = int(item.get("msg_id") or 0)
            cid = int(item.get("chat_id") or chat_id)
            if not mid or (cid == chat_id and mid == current_msg_id) or item.get("deleted"):
                continue
            deleted_count += 1 if _safe_delete(bot, db, cid, mid) else 0
        except Exception as e:
            logger.debug(f"[Codex] 清理广告历史消息异常: uid={uid} item={item} err={e}")
    return deleted_count


def _notify_admin(bot, config: dict, uid: int, uname: str, reason: str, deleted_count: int, muted: bool):
    """通知管理员广告处置结果。"""
    ...
    try:
        ...
        logger.debug(f"[Codex] 广告处置通知管理员失败: uid={uid} err={e}")
...
def enforce_ad_user(
    ...
    if notify_admin:
        _notify_admin(bot, config or {}, uid, uname or str(uid), reason, deleted_count, muted)
    logger.warning(
        f"[Codex] 广告账号处置完成: uid={uid} chat={chat_id} "
        f"muted={muted} blacklisted={blacklisted} deleted={deleted_count}"
    )
    return {
        "code": 200,
        "data": {
